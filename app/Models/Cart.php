...
// **************** ADD TO CART *******************
    ...
    public function add($item, $price,$image,$values) {
        $size_cost = 0;
        $storedItem = ['price' => $item->price, 'image' => $item->image, 'id' => $item->id, 'item' => $item];
        if($item->type == 'Physical')
        {
            ...
            if ($this->items) {
                if (array_key_exists($price.$image.str_replace(str_split(' ,'),'',$values), $this->items)) {
                    $storedItem = $this->items[$price.$image.str_replace(str_split(' ,'),'',$values)];
                    $storedItem['dp'] = 1;
                }
            }
        }
//        $storedItem['qty']++;
        ...
        if(!empty($item->price)){
        $storedItem['prive'] = $item->price[0];
        }
        if(!empty($item->image)){
        $storedItem['image'] = $image;
        }
//        if(!empty($item->id)){
//        $storedItem['id'] = $item->id[0];
//        }
...
//        $storedItem['price'] = $item->price * $storedItem['qty'];
        $this->items[$price.$image.str_replace(str_split(' ,'),'',$values)] = $storedItem;
        $this->totalQty++;
    }
